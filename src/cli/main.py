...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.print_config:  # 若用户请求打印配置。
        print(render_effective_config(bundle, include_sources=True))  # 输出带来源注释的 YAML。
        if args.save_config:  # 同时指定保存时一并处理。
            save_config(bundle, args.save_config)
        return 0  # 打印/保存后直接退出。
    if args.save_config:  # 仅保存配置时也需要提前退出。
        save_config(bundle, args.save_config)
        print(f"Configuration saved to {args.save_config}")  # 为易用性打印确认。
        return 0
    ...
    try:
        ...
        logger.error("fatal pipeline error", error=str(exc))  # 记录错误摘要。
        if config.get("verbose"):
            logger.error("pipeline stack trace", trace=traceback.format_exc())  # 在 verbose 下输出堆栈。
        return 1  # 非零退出表示失败。
