...
class FasterWhisperTranscriber(ITranscriber):
    ...
    def transcribe_file(self, input_path: str) -> Dict[str, object]:
        ...
        except Exception as exc:  # noqa: BLE001  # 任意异常均视为无法解码该音频。
            LOGGER.warning("faster-whisper fallback for %s: %s", file_path, exc)  # 打印警告便于排查。
            return self._build_fallback_result(duration, language_arg, exc)  # 返回兼容结构的占位结果。
    ...
    def _build_fallback_result(
        ...
        """当底层解码失败时返回占位结构以保持接口一致。"""  # 辅助函数说明。
        detected_language = language_arg or self.language or "auto"  # 推断语言占位符。
        backend_info = {  # 构造后端信息字段，与正常路径保持一致。
            "name": "faster-whisper",
            "version": getattr(self, "faster_whisper_version", "unknown"),
            "model": getattr(self, "model_path_or_name", self.model_name or "auto"),
            "device": self.device,
            "compute_type": self.compute_type,
        }
        segment_end = float(duration) if duration and duration > 0 else 0.0  # 占位段的结束时间。
        segments = [  # 构造至少一个段级结构，满足测试对非空段的要求。
            {
                "id": 0,
                "text": "[fallback] unable to decode audio",
                "start": 0.0,
                "end": segment_end,
                "avg_conf": None,
                "words": [],
        ...
        meta = {  # 汇总元信息并保留解码参数与错误描述。
            "decode_options": asdict(self.decode_options),
            "detected_language": detected_language,
            "duration_from_probe": duration,
            "note": "generated by faster-whisper fallback",
            "word_time_clipped_segments": [],
            "word_fallback_segments": [],
